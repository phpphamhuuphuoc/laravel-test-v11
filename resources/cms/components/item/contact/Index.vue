                ...
                <table class="w-full text-left border-b border-slate-200/60">
                    ...
                    <thead v-if="data" ref="dataTable">
                        <tr v-for="item in data" :key="item">
                            ...
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                <router-link :to="{ name: 'cms.item.contact.edit', params: { id: item.id } }" class="font-medium whitespace-nowrap">{{ item.email }}</router-link>
                            </td>
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                    ...
                                    <div class="relative h-5" >
                                        <Dropdown placement="bottom-end">
                                            ...
                                            <DropdownMenu class="w-40">
                                                <DropdownContent>
                                                    <DropdownItem>
                                                        <router-link :to="{ name: 'cms.item.contact.edit', params: { id: item.id } }" class="font-medium whitespace-nowrap">Chỉnh sửa</router-link>
                                                    </DropdownItem>
...
export default {
    ...
    methods: {
        ...
        saveFieldCustomTable() {
            Cookies.set('cms/item/contact', JSON.stringify(this.fieldCustomTable), { expires: 7 });
            this.toggleModalCustomTable();
        },
        getFieldCustomTable() {
            const value = Cookies.get('cms/item/contact');
        ...
        load() {
            return axios
                .get(`${this.baseURL}/cms/api/item/contact`, {
                    params: this.filters,
                })
                .then((response) => {
        ...
        destroy(id) {
            axios.delete(`${this.baseURL}/cms/api/item/contact/${id}`)
            .then((response) => {
        ...
        loadCategory() {
            let that = this;
            return axios
                .get(`${this.baseURL}/cms/api/item/contact/category`)
                .then((response) => {
