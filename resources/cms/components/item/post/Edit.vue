<template>
    <div class="flex flex-col md:h-10 gap-y-3 md:items-center md:flex-row mt-5">
        ...
        <router-link :to="{ name: 'cms.item.post.index' }"
            class="btn btn-secondary ml-auto hover:ring-4">Quay lại</router-link>
        <!-- <a :href="this.baseURL + '/cms/item/post'" class="btn btn-secondary ml-auto hover:ring-4">Quay lại</a> -->
...
export default {
    ...
    methods: {
        ...
        exit() {
            setTimeout(function () {
                window.location.href = `${that.baseURL}/cms/item/post`
            }, 1000);
        },
        update(exit) {
            let that = this;
            axios
                .put(`${this.baseURL}/cms/api/item/post/${this.id}`, this.form)
                .then((response) => {
        ...
        load(){
            let that = this;
            return axios
                .get(`${this.baseURL}/cms/api/item/post/${this.id}`)
                .then((response) => {
                    var responseData = response.data.data;
