<template>
    <div class="flex flex-col md:h-10 gap-y-3 md:items-center md:flex-row mt-5">
        <div class="text-2xl font-medium group-[.mode--light]:text-white">Post</div>
        <div class="flex flex-col sm:flex-row gap-x-3 gap-y-2 md:ml-auto">
            <a :href="`${this.baseURL}/cms/item/post/create`"
                ...
                <table class="w-full text-left border-b border-slate-200/60">
                    ...
                    <thead v-if="data" ref="dataTable">
                        <tr v-for="item in data" :key="item">
                            ...
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                <a :href="this.baseURL + '/cms/item/post/edit/' + item.id">{{ item.title }}</a>
                            </td>
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                    ...
                                    <div class="relative h-5" >
                                        <Dropdown placement="bottom-end">
                                            ...
                                            <DropdownMenu class="w-40">
                                                <DropdownContent>
                                                    <DropdownItem>
                                                        <a :href="this.baseURL + '/cms/item/post/edit/' + item.id">Chỉnh sửa</a>
                                                    </DropdownItem>
...
export default {
    ...
    methods: {
        ...
        saveFieldCustomTable() {
            Cookies.set('cms/item/post', JSON.stringify(this.fieldCustomTable), { expires: 7 });
            this.toggleModalCustomTable();
        },
        getFieldCustomTable() {
            const value = Cookies.get('cms/item/post');
        ...
        load() {
            return axios
                .get(`${this.baseURL}/cms/api/item/post`, {
                    params: this.filters,
                })
                .then((response) => {
        ...
        destroy(id) {
            axios.delete(`${this.baseURL}/cms/api/item/post/${id}`)
            .then((response) => {
        ...
        loadCategory() {
            let that = this;
            return axios
                .get(`${this.baseURL}/cms/api/item/post/category`)
                .then((response) => {
