<template>
    <div class="flex flex-col md:h-10 gap-y-3 md:items-center md:flex-row mt-5">
        ...
        <router-link :to="{ name: 'cms.item.product.index' }"
            class="btn btn-secondary ml-auto hover:ring-4">Quay lại</router-link>
        <!-- <a :href="this.baseURL + '/cms/item/product'" class="btn btn-secondary ml-auto hover:ring-4">Quay lại</a> -->
...
export default {
    ...
    methods: {
        ...
        exit() {
            setTimeout(function () {
                window.location.href = `${that.baseURL}/cms/item/product`
            }, 1000);
        },
        update(exit) {
            let that = this;
            axios
                .put(`${this.baseURL}/cms/api/item/product/${this.id}`, this.form)
                .then((response) => {
        ...
        load(){
            let that = this;
            return axios
                .get(`${this.baseURL}/cms/api/item/product/${this.id}`)
                .then((response) => {
                    var responseData = response.data.data;
