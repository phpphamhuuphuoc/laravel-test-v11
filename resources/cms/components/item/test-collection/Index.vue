<template>
    <div class="flex flex-col md:h-10 gap-y-3 md:items-center md:flex-row mt-5">
        <div class="text-2xl font-medium group-[.mode--light]:text-white">TestCollection</div>
        <div class="flex flex-col sm:flex-row gap-x-3 gap-y-2 md:ml-auto">
            <a :href="`${this.baseURL}/cms/item/test-collection/create`"
                ...
                <table class="w-full text-left border-b border-slate-200/60">
                    ...
                    <thead v-if="data" ref="dataTable">
                        <tr v-for="item in data" :key="item">
                            ...
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                <a :href="this.baseURL + '/cms/item/test-collection/edit/' + item.id">{{ item.title }}</a>
                            </td>
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                <div class="whitespace-nowrap w-40 text-ellipsis overflow-hidden">{{ item.slug }}</div>
                            </td>
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                <div class="whitespace-nowrap w-40 text-ellipsis overflow-hidden">{{ item.desc }}</div>
                            </td>
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                <img class="w-16" :src="item.thumbnail" alt="">
                            </td>
                            <td class="px-5 border-b dark:border-darkmode-300 py-4 border-dashed dark:bg-darkmode-600">
                                <div class="whitespace-nowrap">{{ item.status }}</div>
                            </td>
                                    ...
                                    <div class="relative h-5" >
                                        <Dropdown placement="bottom-end">
                                            ...
                                            <DropdownMenu class="w-40">
                                                <DropdownContent>
                                                    <DropdownItem>
                                                        <a :href="this.baseURL + '/cms/item/test-collection/edit/' + item.id">Chỉnh sửa</a>
                                                    </DropdownItem>
...
export default {
    ...
    methods: {
        ...
        saveFieldCustomTable() {
            Cookies.set('cms/item/test-collection', JSON.stringify(this.fieldCustomTable), { expires: 7 });
            this.toggleModalCustomTable();
        },
        getFieldCustomTable() {
            const value = Cookies.get('cms/item/test-collection');
        ...
        load() {
            return axios
                .get(`${this.baseURL}/cms/api/item/test-collection`, {
                    params: this.filters,
                })
                .then((response) => {
        ...
        destroy(id) {
            axios.delete(`${this.baseURL}/cms/api/item/test-collection/${id}`)
            .then((response) => {
        ...
        loadCategory() {
            let that = this;
            return axios
                .get(`${this.baseURL}/cms/api/item/test-collection/category`)
                .then((response) => {
